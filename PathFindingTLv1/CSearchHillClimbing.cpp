// File:	      CSearchHillCLimbing.cpp
// Author:	      Mark Ince
// Date:	      28-12-2018
// Last Edited:   02-02-2019
//
// Implementation of Search class for Hill Climbing algorithm
//

#include <iostream>
#include <algorithm> 
#include "CSearchHillClimbing.hpp"
...
CSearchHillClimbing::CSearchHillClimbing() // Constructor included even tho empty
{
...
bool CSearchHillClimbing::FindNodeInList(deque <unique_ptr<coords>>& myList, int xCord, int yCord)
{
...
void CSearchHillClimbing::DisplayList(deque<unique_ptr<coords>>& myList)
{
	deque <unique_ptr <coords>>::iterator it; // Create an iterator
...
bool CSearchHillClimbing::FindPath(TerrainMap& terrain, unique_ptr<SNode> start, unique_ptr<SNode> goal, NodeList& path, int xMapSize, int yMapSize)
{
	...
	while (!foundGoal) 	// Until a goal state is found or OpenList is empty
	{
		...
		currentNode = move(openList.front()); // Removes (pop) the first element from OpenList and call it 'current'
		openList.pop_front();
		...
		for (int i = 0; i < VALUE_FOUR; i++) // Check 4 nodes around the current
		{
				...
				(tempNode->y >= 0 && tempNode->y < int(terrain.size())))
			{
				...
				{
					tmpList.push_back(move(tempNode)); // Push node onto the back of the tmpList
													   // Notice the use of a third temporary list here
		...
		// Sort TmpList according to the heuristic values of the elements
		sort(tmpList.begin(), tmpList.end(), [](unique_ptr<coords>& lhs,
			unique_ptr<coords>& rhs) { return lhs->score < rhs->score; });

		// This next while loop adds TmpList to the front of OpenList
		// It does this by removing each element from the BACK of the tmpList and 
		// popping them back onto the FRONT of the openList
		while (!tmpList.empty())
		{
			// Pop each item off the back of the tmpList
			tempNode = move(tmpList.back());
			tmpList.pop_back();
			// Push each item onto the front of the openList
			openList.push_front(move(tempNode));
			tempNode.reset(new coords);
...
CSearchHillClimbing::~CSearchHillClimbing() // Destructor
{
