// File:	      CSearchDepthFirst.cpp
// Author:	      Mark Ince
// Date:	      28-12-2018
// Last Edited:   02-02-2019
//
// Implementation of Search class for Depth-first algorithm
//

#include <iostream>
#include "CSearchDepthFirst.hpp"
...
CSearchDepthFirst::CSearchDepthFirst() // Constructor included even tho empty
{
...
bool CSearchDepthFirst::FindNodeInList(deque <unique_ptr<coords>>& myList, int xCord, int yCord)
{
...
void CSearchDepthFirst::DisplayList(deque<unique_ptr<coords>>& myList)
{
	deque <unique_ptr <coords>>::iterator it; // Create an iterator
...
bool CSearchDepthFirst::FindPath(TerrainMap& terrain, unique_ptr<SNode> start, unique_ptr<SNode> goal, NodeList& path, int xMapSize, int yMapSize)
{
	...
	while (!foundGoal) 	// Until a goal state is found or OpenList is empty 
	{
		...
		for (int i = 0; i < VALUE_FOUR; i++) // Check 4 nodes around the current
		{
				...
				(tempNode->y >= 0 && tempNode->y < int(terrain.size())))
			{
				...
				{
					openList.push_front(move(tempNode)); // Push node onto the FRONT of the openList
														 // Notice the difference here compared to the
					                                     // depth-first search
...
CSearchDepthFirst::~CSearchDepthFirst() // Destructor
{
